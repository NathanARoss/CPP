...
public:
	...
	bool replace(const ItemType& item, const ItemType& replacementItem) override;
	...
	void inorderIterativeTraversal(void visit(ItemType&)) const;
...
int BinaryNodeTree<ItemType>::getHeightHelper(std::shared_ptr<BinaryNode<ItemType>> subTreePtr) const
{
	if (subTreePtr == nullptr)
		...
		return 1 + __max(getHeightHelper(subTreePtr->getLeftChildPtr()),
...
template<class ItemType>
bool BinaryNodeTree<ItemType>::replace(const ItemType& item, const ItemType& replacementItem)
{
	bool found = false;

	//find the pointer to the item.
	std::shared_ptr<BinaryNode<ItemType>> node = findNode(rootPtr, item, found);

	//the order of items does not matter, so overwrite it without reordering
	if (found)
		node->setItem(replacementItem);
...
template<class ItemType>
void BinaryNodeTree<ItemType>::inorderIterativeTraversal(void visit(ItemType&)) const
{
	std::stack<std::shared_ptr<BinaryNode<ItemType>>> pParents;
	...
	while (!done)
	{
		if (node != nullptr)
		{
			//push node before traversing the left subtree
			pParents.push(node);
